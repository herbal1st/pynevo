...
class VisionRenderer:
    ...
    def draw_vision_arc(
        ...
        if len(cone_points) > 2:
            pygame.draw.polygon(
                self.scratchpad,
                self.profile.skin.color_player_vision_arc,
                cone_points
            )
        ...
        for rel_end in ray_endpoints:
            pygame.draw.line(
                self.scratchpad,
                self.profile.skin.color_player_vision_rays,
                rel_origin,
                rel_end,
                1
            )
        ...
        head_line_len: float = (
            self.profile.skin.player_heading_line_length * tile_size
        )
        hx: int = int(rel_origin[0] + (math.cos(heading) * head_line_len))
        hy: int = int(rel_origin[1] + (math.sin(heading) * head_line_len))
        pygame.draw.line(
            self.scratchpad,
            self.profile.skin.color_player_heading_line,
            rel_origin,
            (hx, hy),
            self.profile.skin.player_heading_line_width
        )
